                             ...
                             amrex::Real target_volfrac)
{
    ...
    if (redistribution_type == "FluxRedist")
    {
        ...
        [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
            {
                dUdt_out(i,j,k,n) *= dt;
                dUdt_out(i,j,k,n) += U_in(i,j,k,n);
            }
        );

        // Make sure to call this while dUdt_out is still the full state, not the increment
        Make1DProfile(bx, ncomp, dUdt_out, flag, vfrac,
                      AMREX_D_DECL(fcx, fcy, fcz), bcc, ccc,  d_bcrec_ptr,
                      lev_geom);
amrex::ParallelFor(bx, ncomp,
        [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
            {
