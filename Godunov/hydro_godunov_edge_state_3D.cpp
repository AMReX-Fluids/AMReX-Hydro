                           ...
                           amrex::Array4<int const> const& bc_arr)
{
    ...
    {
        Real uad = umac(i,j,k);
        Real fux = (amrex::Math::abs(uad) < small_vel)? Real(0) : Real(1);
        bool uval = uad >= Real(0);
        ...
        HydroBC::SetEdgeBCsLo(0, i, j, k, n, qnph, lo, hi, umac(i,j,k), bc.lo(0), dlo.x, is_velocity);
        HydroBC::SetEdgeBCsHi(0, i, j, k, n, qnph, lo, hi, umac(i,j,k), bc.hi(0), dhi.x, is_velocity);

        xlo(i,j,k,n) = lo;
        xhi(i,j,k,n) = hi;
        Real st = (uval) ? lo : hi;
        Imx(i,j,k,n) = fux*st + (Real(1) - fux)*Real(0.5)*(hi + lo);

    },
    yebox, ncomp, [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
    {
        Real vad = vmac(i,j,k);
        Real fuy = (amrex::Math::abs(vad) < small_vel)? Real(0) : Real(1);
        bool vval = vad >= Real(0);
        ...
        HydroBC::SetEdgeBCsLo(1, i, j, k, n, qnph, lo, hi, vmac(i,j,k), bc.lo(1), dlo.y, is_velocity);
        HydroBC::SetEdgeBCsHi(1, i, j, k, n, qnph, lo, hi, vmac(i,j,k), bc.hi(1), dhi.y, is_velocity);

        ylo(i,j,k,n) = lo;
        yhi(i,j,k,n) = hi;
        Real st = (vval) ? lo : hi;
        Imy(i,j,k,n) = fuy*st + (Real(1) - fuy)*Real(0.5)*(hi + lo);
    },
    zebox, ncomp, [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
    {
        Real wad = wmac(i,j,k);
        Real fuz = (amrex::Math::abs(wad) < small_vel) ? Real(0) : Real(1);
        bool wval = wad >= Real(0);
        Real lo = Ipz(i,j,k-1,n);
        Real hi = Imz(i,j,k  ,n);

        if (use_forces_in_trans && fq)
        {
            lo += Real(0.5)*l_dt*fq(i,j,k-1,n);
            hi += Real(0.5)*l_dt*fq(i,j,k  ,n);
        }

        const auto bc = HydroBC::getBC(i, j, k, n, domain, pbc, bc_arr);

        HydroBC::SetEdgeBCsLo(2, i, j, k, n, qnph, lo, hi, wmac(i,j,k), bc.lo(2), dlo.z, is_velocity);
        HydroBC::SetEdgeBCsHi(2, i, j, k, n, qnph, lo, hi, wmac(i,j,k), bc.hi(2), dhi.z, is_velocity);

        zlo(i,j,k,n) = lo;
        zhi(i,j,k,n) = hi;
        Real st = (wval) ? lo : hi;
        Imz(i,j,k,n) = fuz*st + (Real(1) - fuz)*Real(0.5)*(hi + lo);
    }
    );
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
    {
        ...
        HydroBC::SetEdgeBCsLo(2, i, j, k, n, qnph, l_zylo, l_zyhi, wmac(i,j,k), bc.lo(2), dlo.z, is_velocity);
        HydroBC::SetEdgeBCsHi(2, i, j, k, n, qnph, l_zylo, l_zyhi, wmac(i,j,k), bc.hi(2), dhi.z, is_velocity);

        Real wad = wmac(i,j,k);
        Real st = (wad >= Real(0)) ? l_zylo : l_zyhi;
        Real fu = (amrex::Math::abs(wad) < small_vel) ? Real(0) : Real(1);
        zylo(i,j,k,n) = fu*st + (Real(1) - fu) * Real(0.5) * (l_zyhi + l_zylo);
    },
    Box(yzlo), ncomp,
    [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
    {
        ...
        Real fu = (amrex::Math::abs(vad) < small_vel) ? Real(0) : Real(1);
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
    {
        ...
        stl += ( - (Real(0.5)*dtdx) * quxl
                 ...
                 - (Real(0.5)*dtdz)*(zylo(i-1,j  ,k+1,n)*wmac(i-1,j  ,k+1)
        ...
        sth += ( - (Real(0.5)*dtdx) * quxh
                 ...
                 - (Real(0.5)*dtdz)*(zylo(i,j  ,k+1,n)*wmac(i,j  ,k+1)
        ...
        const auto bc = HydroBC::getBC(i, j, k, n, domain, pbc, bc_arr);
        HydroBC::SetEdgeBCsLo(0, i, j, k, n, qnph, stl, sth, umac(i,j,k), bc.lo(0), dlo.x, is_velocity);
        HydroBC::SetEdgeBCsHi(0, i, j, k, n, qnph, stl, sth, umac(i,j,k), bc.hi(0), dhi.x, is_velocity);

        if (!allow_inflow_on_outflow) {
            if ( (i==dlo.x) && (bc.lo(0) == BCType::foextrap || bc.lo(0) == BCType::hoextrap) )
            {
                if ( umac(i,j,k) >= Real(0) && n==XVEL && is_velocity )  sth = amrex::min(sth,Real(0));
                stl = sth;
            }
            if ( (i==dhi.x+1) && (bc.hi(0) == BCType::foextrap || bc.hi(0) == BCType::hoextrap) )
            {
                if ( umac(i,j,k) <= Real(0) && n==XVEL && is_velocity ) stl = amrex::max(stl,Real(0));
                 sth = stl;
            }
        }
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
    {
        ...
        HydroBC::SetEdgeBCsLo(0, i, j, k, n, qnph, l_xzlo, l_xzhi, umac(i,j,k), bc.lo(0), dlo.x, is_velocity);
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
    {
        ...
        HydroBC::SetEdgeBCsLo(2, i, j, k, n, qnph, l_zxlo, l_zxhi, wmac(i,j,k), bc.lo(2), dlo.z, is_velocity);
        HydroBC::SetEdgeBCsHi(2, i, j, k, n, qnph, l_zxlo, l_zxhi, wmac(i,j,k), bc.hi(2), dhi.z, is_velocity);

        Real wad = wmac(i,j,k);
        Real st = (wad >= Real(0)) ? l_zxlo : l_zxhi;
        Real fu = (amrex::Math::abs(wad) < small_vel) ? Real(0) : Real(1);
        zxlo(i,j,k,n) = fu*st + (Real(1) - fu) * Real(0.5) * (l_zxhi + l_zxlo);
    });
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
    {
        ...
        stl += ( - (Real(0.5)*dtdy) * qvyl
                 ...
                 - (Real(0.5)*dtdz)*(zxlo(i  ,j-1,k+1,n)*wmac(i  ,j-1,k+1)
        ...
        sth += ( - (Real(0.5)*dtdy) * qvyh
                 ...
                 - (Real(0.5)*dtdz)*(zxlo(i  ,j,k+1,n)*wmac(i  ,j,k+1)
        ...
        const auto bc = HydroBC::getBC(i, j, k, n, domain, pbc, bc_arr);
        HydroBC::SetEdgeBCsLo(1, i, j, k, n, qnph, stl, sth, vmac(i,j,k), bc.lo(1), dlo.y, is_velocity);
        HydroBC::SetEdgeBCsHi(1, i, j, k, n, qnph, stl, sth, vmac(i,j,k), bc.hi(1), dhi.y, is_velocity);
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
    {
        ...
        HydroBC::SetEdgeBCsLo(0, i, j, k, n, qnph, l_xylo, l_xyhi, umac(i,j,k), bc.lo(0), dlo.x, is_velocity);
        HydroBC::SetEdgeBCsHi(0, i, j, k, n, qnph, l_xylo, l_xyhi, umac(i,j,k), bc.hi(0), dhi.x, is_velocity);

        Real uad = umac(i,j,k);
        Real st = (uad >= Real(0)) ? l_xylo : l_xyhi;
        Real fu = (amrex::Math::abs(uad) < small_vel) ? Real(0) : Real(1);
        xylo(i,j,k,n) = fu*st + (Real(1) - fu) * Real(0.5) * (l_xyhi + l_xylo);
    },
    Box(yxlo), ncomp,
    [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
    {
        ...
        HydroBC::SetEdgeBCsLo(1, i, j, k, n, qnph, l_yxlo, l_yxhi, vmac(i,j,k), bc.lo(1), dlo.y, is_velocity);
        HydroBC::SetEdgeBCsHi(1, i, j, k, n, qnph, l_yxlo, l_yxhi, vmac(i,j,k), bc.hi(1), dhi.y, is_velocity);

        Real vad = vmac(i,j,k);
        Real st = (vad >= Real(0)) ? l_yxlo : l_yxhi;
        Real fu = (amrex::Math::abs(vad) < small_vel) ? Real(0) : Real(1);
        yxlo(i,j,k,n) = fu*st + (Real(1) - fu) * Real(0.5) * (l_yxhi + l_yxlo);
    });
    //
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
    {
        ...
        stl += ( -(Real(0.5)*dtdz) * qwzl
                 -(Real(0.5)*dtdx)*(xylo(i+1,j  ,k-1,n)*umac(i+1,j  ,k-1)
                             -xylo(i  ,j  ,k-1,n)*umac(i  ,j  ,k-1))
                 -(Real(0.5)*dtdy)*(yxlo(i  ,j+1,k-1,n)*vmac(i  ,j+1,k-1)
                             -yxlo(i  ,j  ,k-1,n)*vmac(i  ,j  ,k-1)) );

        // Here we adjust for non-conservative by removing the q divu contribution to get
        //     q + dz/2 q_z - dt/2 ( div (uvec q) - q divu ) which is equivalent to
        // --> q + dz/2 q_z - dt/2 ( uvec dot grad q)
        stl += (!iconserv[n])               ? Real(0.5)*l_dt* q(i,j,k-1,n)*divu(i,j,k-1) : Real(0);

        stl += (!use_forces_in_trans && fq) ? Real(0.5)*l_dt*fq(i,j,k-1,n) : Real(0);

        // High side
        Real qwzh = (wmac(i,j,k+1) - wmac(i,j,k)) * q(i,j,k,n);
        sth += ( - (Real(0.5)*dtdz) * qwzh
                 - (Real(0.5)*dtdx)*(xylo(i+1,j  ,k,n)*umac(i+1,j  ,k)
                               -xylo(i  ,j  ,k,n)*umac(i  ,j  ,k))
                 - (Real(0.5)*dtdy)*(yxlo(i  ,j+1,k,n)*vmac(i  ,j+1,k)
        ...
        const auto bc = HydroBC::getBC(i, j, k, n, domain, pbc, bc_arr);
        HydroBC::SetEdgeBCsLo(2, i, j, k, n, qnph, stl, sth, wmac(i,j,k), bc.lo(2), dlo.z, is_velocity);
        HydroBC::SetEdgeBCsHi(2, i, j, k, n, qnph, stl, sth, wmac(i,j,k), bc.hi(2), dhi.z, is_velocity);

        if (!allow_inflow_on_outflow) {
            if ( (k==dlo.z) && (bc.lo(2) == BCType::foextrap || bc.lo(2) == BCType::hoextrap) )
            {
                if ( wmac(i,j,k) >= Real(0) && n==ZVEL && is_velocity ) sth = amrex::min(sth,Real(0));
                stl = sth;
            }
            if ( (k==dhi.z+1) && (bc.hi(2) == BCType::foextrap || bc.hi(2) == BCType::hoextrap) )
            {
                if ( wmac(i,j,k) <= Real(0) && n==ZVEL && is_velocity ) stl = amrex::max(stl,Real(0));
                sth = stl;
            }
        }

        Real temp = (wmac(i,j,k) >= Real(0)) ? stl : sth;
        temp = (amrex::Math::abs(wmac(i,j,k)) < small_vel) ? Real(0.5)*(stl + sth) : temp;
        zedge(i,j,k,n) = temp;
    });
