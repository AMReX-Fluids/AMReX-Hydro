...
#if AMREX_USE_EB
#include <hydro_mol_eb_edge_state_K.H>
#endif
                       ...
                       const        BCRec * d_bcrec_ptr)
{
        ...
        (has_extdir_or_ho_hi and domain_ihi <= ubx.bigEnd(0)))
    {
        ...
        {
            xedge(i,j,k,n) = hydro_xedge_state_mol_extdir( i, j, k, n, q, umac,
                                                          d_bcrec_ptr,
    ...
    else
    {
        ...
        AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
        {
            xedge(i,j,k,n) = hydro_xedge_state_mol( i, j, k, n, q, umac);
        });
    }
        ...
        (has_extdir_or_ho_hi and domain_jhi <= vbx.bigEnd(1)))
    {
        ...
        AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
        {
            yedge(i,j,k,n) = hydro_yedge_state_mol_extdir( i, j, k, n, q, vmac,
                                                          d_bcrec_ptr,
    ...
    else
    {
        ...
        AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
        {
            yedge(i,j,k,n) = hydro_yedge_state_mol( i, j, k, n, q, vmac );
        });
    }
        ...
        (has_extdir_or_ho_hi and domain_khi <= wbx.bigEnd(2)))
    {
        ...
        AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
        {
            zedge(i,j,k,n) = hydro_zedge_state_mol_extdir( i, j, k, n, q, wmac,
                                                          d_bcrec_ptr,
    ...
    else
    {
        ...
        AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
        {
            zedge(i,j,k,n) = hydro_zedge_state_mol( i, j, k, n, q, wmac );
        });
    }
                           ...
                           Array4<EBCellFlag const> const& flag)
{
...
#endif
    {
        ...
        AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
        {
           ...
           {
               xedge(i,j,k,n) = hydro_eb_xedge_state_mol_extdir( AMREX_D_DECL(i, j, k), n, q, umac,
        ...
        AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
        {
            ...
            {
                yedge(i,j,k,n) = hydro_eb_yedge_state_mol_extdir( AMREX_D_DECL(i, j, k), n, q, vmac,
								 AMREX_D_DECL(fcx,fcy,fcz), ccc, vfrac,
                                                                 flag, d_bcrec_ptr, domain, order );
            }
            else
        ...
        AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
        {
            ...
            {
                zedge(i,j,k,n) = hydro_eb_zedge_state_mol_extdir( i, j, k, n, q, wmac,
								 AMREX_D_DECL(fcx,fcy,fcz), ccc, vfrac,
                                                                 flag, d_bcrec_ptr, domain, order );
            }
            else
    ...
    else // We assume below that the stencil does not need to use hoextrap or extdir boundaries
    {
        ...
        AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
        {
           if (flag(i,j,k).isConnected(-1,0,0))
           {
               xedge(i,j,k,n) = hydro_eb_xedge_state_mol( AMREX_D_DECL(i, j, k), n, q, umac,
                                                         AMREX_D_DECL(fcx,fcy,fcz), ccc, vfrac,
                                                         flag, order);
           }
           else
        ...
        AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
        {
            if (flag(i,j,k).isConnected(0,-1,0))
            {
                yedge(i,j,k,n) = hydro_eb_yedge_state_mol( AMREX_D_DECL(i, j, k), n, q, vmac,
                                                          AMREX_D_DECL(fcx,fcy,fcz), ccc, vfrac,
                                                          flag, order );
            }
            else
        ...
        AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
        {
            if (flag(i,j,k).isConnected(0,0,-1))
            {
                zedge(i,j,k,n) = hydro_eb_zedge_state_mol( AMREX_D_DECL(i, j, k), n, q, wmac,
                                                          AMREX_D_DECL(fcx,fcy,fcz), ccc, vfrac,
                                                          flag, order );
            }
            else
