...
Pre-MAC (Godunov::ExtrapVelToFacesBox )
---------------------------------------
...
.. math::
   ...
   \begin{aligned}
   u_L &=& u_{i-1,j,k} + \frac{\Delta x}{2} {u_x}_{i-1,j,k} + \frac{\Delta t}{2} {u_t}_{i-1,j,k} , \\
   u_R &=& u_{i,j,k}   - \frac{\Delta x}{2} {u_x}_{i,j,k} +  \frac{\Delta t}{2} {u_t}_{i,j,k} , \end{aligned}

where :math:`u_x` are the (limited) slopes in the x-direction and we will discuss 
:math:`u_t` below.
